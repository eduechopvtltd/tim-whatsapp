...
export default function App() {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/api/config`).then(r => r.json()).then(data => {
      ...
      if (data.PHONE_NUMBER_ID && data.WABA_ID && data.ACCESS_TOKEN) {
        setIsConnected(true);
        fetch(`${API_BASE}/api/templates`).then(r => r.json()).then(tpls => {
          if (Array.isArray(tpls)) { setMetaSynced(true); setTemplates(tpls); }
        }).catch(() => setMetaSynced(false));
      }
    }).catch(() => {});
    fetch(`${API_BASE}/api/templates`).then(r => r.json()).then(d => { if (Array.isArray(d)) setTemplates(d); }).catch(() => {});
    fetch(`${API_BASE}/api/active-job`).then(r => r.json()).then(data => {
      if (data.jobId) { setJobId(data.jobId); setJobStatus(data.status || data); setActiveTab('status'); }
    }).catch(() => {});
    fetch(`${API_BASE}/api/history`).then(r => r.json()).then(d => { if (Array.isArray(d)) setHistoryData(d); }).catch(() => {});
  }, []);

  // REAL-TIME SYNC CHECK
  useEffect(() => {
    const checkConn = async () => {
      const isConfigured = !!(config.PHONE_NUMBER_ID && config.WABA_ID && config.ACCESS_TOKEN);
      ...
      try {
        ...
        if (res.ok) {
           ...
           setMetaSynced(true);
        } else { setMetaSynced(false); }
      } catch (e) { setMetaSynced(false); }
    };
    checkConn();
    const inv = setInterval(checkConn, 30000);
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (data.status === 'Completed' || data.status === 'Stopped') {
          ...
          fetch(`${API_BASE}/api/history`).then(r => r.json()).then(d => { if (Array.isArray(d)) setHistoryData(d); });
        }
      } catch (e) {}
    }, 2000);
  ...
  useEffect(() => {
    const fetchChats = () => {
      fetch(`${API_BASE}/api/chats`).then(r => r.json()).then(d => { if (Array.isArray(d)) setChats(d); }).catch(() => {});
    };
  ...
  useEffect(() => {
    if (activeTab === 'inbox' && activeChatPhone) {
      const fetchHistory = () => {
        fetch(`${API_BASE}/api/chats/${activeChatPhone}`).then(r => r.json()).then(d => { if (Array.isArray(d)) setActiveChatHistory(d); }).catch(() => {});
      };
      fetchHistory();
  ...
  const stats = useMemo(() => {
    ...
    historyData.forEach(job => {
      const sent = job.results ? job.results.filter(r => r.status?.includes('Sent')).length : 0;
      const failed = job.results ? job.results.filter(r => r.status?.includes('Failed')).length : 0;
      totalSent += sent;
      totalFailed += failed;
      
      // Weekly Activity (Aggregate by day of week)
      if (job.createdAt) {
        const date = new Date(job.createdAt);
        const day = (date.getDay() + 6) % 7; // Convert Sun=0..Sat=6 to Mon=0..Sun=6
        dailyCounts[day] += sent;
      }
    });
    
    const max = Math.max(...dailyCounts, 1);
    const weeklyActivity = dailyCounts.map(count => ({
      height: (count / max) * 100,
      label: ['M', 'T', 'W', 'T', 'F', 'S', 'S'][dailyCounts.indexOf(count)], // Initial labels
      count: count
    }));

    // Fix labels to be unique correctly mapping days
  ...
  const headerInfo = useMemo(() => {
    const h = selectedTpl?.componentsData?.header;
    if (!h) return null;
    return h;
  }, [selectedTpl]);
  ...
  const templatePreviewText = useMemo(() => {
    ...
    const bodyVars = selectedTpl.componentsData?.body?.variables || [];
    if (bodyVars.length === 0) return 'No body variables in this template.';
    return `This template uses ${bodyVars.length} variable(s): ${bodyVars.join(', ')}`;
  }, [selectedTpl]);
  ...
  const filteredHistory = useMemo(() => {
    ...
    return historyData.filter(job => 
      (job.templateName || '').toLowerCase().includes(debouncedHistorySearch.toLowerCase()) ||
      (job.id || '').includes(debouncedHistorySearch)
    );
  ...
  useEffect(() => {
    const timer = setTimeout(() => setDebouncedHistorySearch(historySearchTerm), 300);
    return () => clearTimeout(timer);
  }, [historySearchTerm]);

  const handleRegisterPhone = async () => {
    if (!registrationPin || registrationPin.length !== 6) {
      setStatus('Please enter a 6-digit PIN.');
      return;
    }
    setIsRegistering(true);
    setStatus('Attempting Meta registration...');
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        setStatus('Registration Successful! Refreshing templates...');
        await handleRefreshTemplates();
      } else {
        setStatus(`Registration Error: ${data.error || 'Failed'}`);
      }
    } catch (e) {
      setStatus('Register failed — check server logs.');
    } finally {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      const resp = await fetch(`${API_BASE}/api/upload-csv`, { method: 'POST', body: formData });
      ...
      setStatus('Upload failed — check your CSV file');
      setFile(null); setCsvData([]); setCsvHeaders([]);
    }
  };
  ...
  const handleMediaUpload = async (e) => {
    ...
    setStatus('Uploading media to Meta...');
  ...
  const handleRefreshTemplates = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (Array.isArray(data)) { setTemplates(data); setStatus(`Synced ${data.length} templates`); }
    } catch (err) { setStatus('Failed to refresh templates'); }
    ...
    try {
      const resp = await fetch(`${API_BASE}/api/send`, {
        ...
        body: JSON.stringify({
          ...
          messageType: messageType === 'custom' ? 'text' : 'template',
          customMessage, mapping, allowDuplicates
        })
      });
      const data = await resp.json();
  ...
  const handleSendReply = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const histRes = await fetch(`${API_BASE}/api/chats/${activeChatPhone}`);
        const histData = await histRes.json();
        setActiveChatHistory(histData);
      } else {
    ...
    <div className="min-h-screen flex bg-bg-base text-slate-300">
      ...
      <aside className={cn(
        ...
        <div className="px-4 mb-8 flex items-center justify-between">
           <div className="flex items-center gap-2">
             ...
             <h1 className="text-lg font-bold">TIM WebAPI</h1>
           </div>
        ...
        <div className="mt-auto px-2">
           <div className="p-3 rounded-xl bg-bg-surface border border-border-dim flex items-center gap-3">
              <div className="w-8 h-8 rounded-full bg-emerald-500/10 flex items-center justify-center text-emerald-500 font-bold border border-emerald-500/20 text-xs">S</div>
              <div className="flex-1 min-w-0">
                 <p className="text-xs font-bold text-white">Sahil Admin</p>
                 <p className="text-[10px] text-slate-500">TIM Instance</p>
              </div>
           </div>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden w-full min-w-0">
                         ...
                         <div className="simple-card h-48 lg:h-64 flex items-end justify-between gap-1.5 lg:gap-3 px-4 lg:px-6 pt-10 pb-2">
                             {stats.weeklyActivity.map((dayData, i) => (
                                <div key={i} className="flex-1 flex flex-col items-center gap-2 h-full justify-end">
                                   <motion.div 
                                      initial={{ height: 0 }} 
                                      animate={{ height: `${Math.max(dayData.height, 5)}%` }} 
                                      className="w-full bg-emerald-500/20 rounded-t-sm lg:rounded-t-md relative group min-h-[4px]"
                                   >
                      ...
                      <div className="w-full lg:w-96 space-y-4">
                         ...
                         <div className="space-y-2 lg:space-y-3">
                            {historyData.slice(0, 4).map(job => (
                               <div key={job.id} className="p-3 lg:p-4 rounded-xl bg-bg-surface border border-border-dim flex items-center justify-between group hover:border-emerald-500/10 transition-all">
                                  <div className="flex items-center gap-3 min-w-0">
                                     <div className={cn("w-2 h-2 rounded-full shrink-0", job.status === 'Completed' ? "bg-emerald-500" : "bg-amber-500")} />
                                     <div className="min-w-0">
                                        <p className="text-xs font-bold text-white truncate">{job.templateName}</p>
                                        <p className="text-[10px] text-slate-500">{job.total} contacts</p>
                                     </div>
                                  </div>
                ...
                <motion.div key="send" {...PAGE_TRANSITION} className="max-w-4xl space-y-6 lg:space-y-8">
                   <div className="simple-card space-y-6">
                      <div className="flex items-center gap-3 lg:gap-4 border-b border-border-dim pb-4 lg:pb-6">
                         ...
                         <div>
                            <h3 className="text-base lg:text-lg font-bold">1. Upload Contacts</h3>
                         ...
                         <div className={cn("h-32 lg:h-40 rounded-2xl border-2 border-dashed flex flex-col items-center justify-center gap-2 transition-all", file ? "border-emerald-500/30 bg-emerald-500/5" : "border-border-dim bg-white/[0.01] hover:border-emerald-500/20")}>
                            {file ? (
                               ...
                               <div className="text-center">
                                  <Plus size={22} className="text-slate-600 mb-2 mx-auto" weight="bold" />
                                  <p className="text-[10px] lg:text-xs font-bold text-slate-500 uppercase tracking-widest">Click to Upload CSV</p>
                                </div>
                            )}
                         </div>
                   ...
                   <div className="simple-card space-y-6">
                      <div className="flex items-center gap-3 lg:gap-4 border-b border-border-dim pb-4 lg:pb-6">
                         ...
                         <div className="flex-1">
                            <h3 className="text-base lg:text-lg font-bold">2. Choose Message</h3>
                          ...
                          {messageType === 'template' ? (
                             <div className="space-y-5">
                                <div className="flex gap-3 items-end">
                                   ...
                                   <button onClick={handleRefreshTemplates} disabled={refreshingTemplates} className={cn("simple-btn bg-white/5 border border-border-dim text-slate-400 hover:text-emerald-500 hover:border-emerald-500/20 h-[46px] lg:h-[52px] px-3.5 mb-[1px]", refreshingTemplates && "animate-pulse")} title="Refresh templates from Meta">
                                      <ArrowsClockwise size={20} className={refreshingTemplates ? "animate-spin" : ""} />
                                   </button>
                                   {selectedTpl && (
                                     <button onClick={() => setShowPreview(!showPreview)} className="simple-btn bg-white/5 border border-border-dim text-slate-400 hover:text-emerald-500 hover:border-emerald-500/20 h-[46px] lg:h-[52px] px-3.5 mb-[1px]" title="Preview template">
                                        <Eye size={20} />
                                     </button>
                                   )}
                                </div>
                                  ...
                                  <div className="space-y-4">
                                    ...
                                    <div className="relative">
                                      ...
                                      <div className="relative flex justify-center text-[8px] font-bold uppercase tracking-[0.2em]">
                                        <span className="bg-bg-surface px-2 text-slate-600">OR UPLOAD LOCAL IMAGE</span>
                                      </div>
                                    </div>

                                    <div className="flex items-center gap-3">
                                       <label className={cn(
                                         "flex-1 flex items-center justify-center gap-2 p-3 rounded-xl border border-dashed transition-all cursor-pointer",
                                         uploadedMediaId ? "border-emerald-500/30 bg-emerald-500/5 text-emerald-500" : "border-border-dim bg-white/[0.02] hover:border-emerald-500/20 text-slate-500"
                                       )}>
                                          {isUploadingMedia ? (
                                          ...
                                          <span className="text-[10px] font-bold uppercase tracking-wider">
                                            {isUploadingMedia ? 'Uploading...' : uploadedMediaId ? 'Image Linked' : 'Upload Local Image'}
                                          </span>
                                          <input type="file" className="hidden" accept="image/*" onChange={handleMediaUpload} disabled={isUploadingMedia} />
                                       </label>
                                       {uploadedMediaId && (
                                         <button 
                                           onClick={() => { setUploadedMediaId(null); setMapping(prev => { const n = {...prev}; delete n.header_media_url; return n; }); }}
                                    ...
                                    {uploadedMediaId && (
                                      <p className="text-[9px] text-emerald-500/60 ml-1 font-mono">Meta ID: {uploadedMediaId}</p>
                                    )}
                                  </div>
                                )}
                                {headerInfo?.type === 'TEXT' && headerInfo.variables?.length > 0 && csvHeaders.length > 0 && (
                                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    {headerInfo.variables.map((v, i) => (
                                      <FieldSelect key={i} label={`Header: ${v}`} value={mapping[`header_${v}`] || ''} onChange={val => setMapping({...mapping, [`header_${v}`]: val})} options={csvHeaders} />
                                    ))}
                                  </div>
                                )}
                                {bodyVariables.length > 0 && csvHeaders.length > 0 && (
                                  <div className="space-y-3">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                       {bodyVariables.map((varName, i) => (
                                          <FieldSelect key={i} label={varName} value={mapping[`body_${varName}`] || mapping[varName] || ''} onChange={v => setMapping({...mapping, [`body_${varName}`]: v, [varName]: v})} options={csvHeaders} />
                                       ))}
                                    </div>
                                  </div>
                                )}
                      ...
                      {status && (
                        <div className="p-3 rounded-xl bg-white/[0.02] border border-border-dim flex items-center gap-2 text-xs text-slate-400">
                           <WarningCircle size={14} className="text-emerald-500 shrink-0" />{status}
                        </div>
                      )}
                   ...
                   {jobStatus ? (
                     <>
                       <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4">
                          ...
                          <MiniStat label="Sent" value={jobStatus.results?.filter(r => r.status?.includes('Sent')).length || 0} color="text-emerald-500" />
                          ...
                          <div className="flex items-center justify-between flex-wrap gap-3">
                             ...
                             <div className="flex gap-2">
                               {(jobStatus.status === 'Running' || jobStatus.status === 'Paused') && (
                                 <>
                                    <button onClick={() => fetch(`${API_BASE}/api/${jobStatus.status === 'Running' ? 'pause' : 'resume'}/${jobId}`, { method: 'POST' })} className="p-2 hover:bg-white/5 rounded-lg text-slate-400 transition-colors" title={jobStatus.status === 'Running' ? 'Pause' : 'Resume'}>
                                       {jobStatus.status === 'Running' ? <Pause size={18} /> : <Play size={18} />}
                                    </button>
                                    <button onClick={() => fetch(`${API_BASE}/api/stop/${jobId}`, { method: 'POST' })} className="p-2 hover:bg-red-500/10 rounded-lg text-slate-400 hover:text-red-500 transition-colors" title="Stop">
                                       <Square size={18} />
                                    </button>
                                 </>
                               )}
                               {(jobStatus.status === 'Completed' || jobStatus.status === 'Stopped') && (
                                 <>
                                    <button onClick={handleExportCSV} className="simple-btn bg-white/5 border border-border-dim text-slate-300 hover:text-emerald-500 hover:border-emerald-500/20 flex items-center gap-2 text-xs">
                                       <DownloadSimple size={16} /> Download CSV
                                    </button>
                                    <button onClick={handleRestart} className="simple-btn bg-white/5 border border-border-dim text-slate-300 hover:text-emerald-500 hover:border-emerald-500/20 flex items-center gap-2 text-xs">
                                       <ArrowCounterClockwise size={16} /> New Campaign
                                    </button>
                                 </>
                               )}
                             </div>
                          ...
                          <div className="max-h-72 lg:max-h-80 overflow-y-auto space-y-1.5 pr-1">
                             {filteredResults.map((res, i) => (
                                 <div key={i} onClick={() => setSelectedResult(res)} className="flex items-center justify-between p-2.5 lg:p-3 rounded-xl bg-white/[0.01] border border-border-dim text-[10px] lg:text-[11px] font-medium cursor-pointer hover:bg-white/[0.03] transition-colors group">
                                    ...
                                    <div className="flex items-center gap-3">
                                       <span className={cn("truncate max-w-[140px] lg:max-w-[200px] text-right shrink-0 ml-2", res.status?.includes('Sent') ? "text-emerald-500" : res.status?.includes('Skip') ? "text-amber-500" : "text-red-500")}>{res.status}</span>
                   ...
                   <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 px-1">
                      ...
                      <div className="flex items-center gap-3 w-full sm:w-auto">
                        ...
                        {historyData.length > 0 && !expandedHistoryJob && (
                          <button onClick={() => { if(confirm('Wipe all history?')) fetch(`${API_BASE}/api/history/clear`, { method: 'POST' }).then(() => setHistoryData([])) }} className="text-[10px] font-bold text-red-500/50 hover:text-red-500 uppercase tracking-widest whitespace-nowrap">Clear All</button>
                        )}
                      </div>
                   </div>
                   ...
                   {expandedHistoryJob ? (
                     <div className="space-y-6">
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                           <MiniStat label="Total Contacts" value={expandedHistoryJob.total} />
                           <MiniStat label="Sent Success" value={expandedHistoryJob.results?.filter(r => r.status?.includes('Sent')).length || 0} color="text-emerald-500" />
                           <MiniStat label="Failed" value={expandedHistoryJob.results?.filter(r => r.status?.includes('Failed')).length || 0} color="text-red-500" />
                           <MiniStat label="Skipped" value={expandedHistoryJob.results?.filter(r => r.status?.includes('Skipped')).length || 0} color="text-amber-500" />
                        </div>
                        <div className="simple-card">
                          <div className="flex justify-between items-center mb-6">
                             <div>
                                <h4 className="text-xs font-bold text-white uppercase tracking-widest">{expandedHistoryJob.templateName}</h4>
                                <p className="text-[10px] text-slate-500 mt-1">{new Date(expandedHistoryJob.createdAt).toLocaleString()}</p>
                             </div>
                             <button onClick={() => handleExportHistoryCSV(expandedHistoryJob)} className="simple-btn bg-white/5 border border-border-dim text-slate-300 hover:text-emerald-500 flex items-center gap-2 text-[10px] px-4">
                     ...
                     <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
                        {filteredHistory.map(job => {
                           ...
                           const sent = results.filter(r => r.status?.includes('Sent')).length || 0;
                           const failed = results.filter(r => r.status?.includes('Failed')).length || 0;
                           const rate = job.total > 0 ? (sent / job.total * 100).toFixed(0) : 0;
                           
                           return (
                             <div key={job.id} onClick={() => setExpandedHistoryJob(job)} className="simple-card group cursor-pointer hover:border-emerald-500/20 transition-all">
                                <div className="flex justify-between items-start mb-4">
                                   ...
                                   <div className="text-right">
                                      <span className="text-[10px] font-bold text-slate-600">#{job.id?.slice(-6)}</span>
                                      <p className="text-[9px] text-slate-500 font-bold uppercase mt-1">{job.status || 'Completed'}</p>
                                   </div>
                                </div>
                                <h4 className="text-xs lg:text-sm font-bold text-white mb-1 truncate">{job.templateName}</h4>
                                <p className="text-[10px] text-slate-500 mb-6">{new Date(job.createdAt).toLocaleDateString(undefined, {month:'short', day:'numeric', year:'numeric', hour:'2-digit', minute:'2-digit'})}</p>
                                
                                <div className="grid grid-cols-3 gap-2 mb-6">
                                   <div className="text-center p-2 rounded-lg bg-white/[0.02] border border-border-dim">
                                      <p className="text-[8px] text-slate-600 font-bold uppercase mb-1">Sent</p>
                                      <p className="text-xs font-bold text-emerald-500">{sent}</p>
                                   </div>
                                   <div className="text-center p-2 rounded-lg bg-white/[0.02] border border-border-dim">
                                      <p className="text-[8px] text-slate-600 font-bold uppercase mb-1">Fail</p>
                                      <p className="text-xs font-bold text-red-500">{failed}</p>
                                   </div>
                                   <div className="text-center p-2 rounded-lg bg-white/[0.02] border border-border-dim">
                                      <p className="text-[8px] text-slate-600 font-bold uppercase mb-1">Total</p>
                                      <p className="text-xs font-bold text-white">{job.total}</p>
                                   </div>
                                </div>
                                ...
                                <div className="pt-4 border-t border-border-dim flex justify-between items-center">
                                   ...
                                   <button onClick={(e) => { e.stopPropagation(); handleExportHistoryCSV(job); }} className="p-2 hover:bg-emerald-500/10 rounded-lg text-slate-500 hover:text-emerald-500 transition-all">
                        ...
                        {filteredHistory.length === 0 && (
                          <div className="col-span-full text-center py-40">
                             ...
                             <p className="text-xs font-bold text-slate-700 uppercase tracking-widest">No matching history</p>
                          </div>
                        )}
                     </div>
                   )}
                   ...
                   <div className="w-80 flex flex-col gap-4 shrink-0">
                      ...
                      <div className="flex-1 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                         {chats.length > 0 ? chats.map(chat => (
                            <button 
                              key={chat.phone} 
                              onClick={() => { setActiveChatPhone(chat.phone); setActiveChatHistory([]); }}
                              className={cn(
                                "w-full text-left p-4 rounded-2xl border transition-all group relative",
                                activeChatPhone === chat.phone ? "bg-emerald-500/10 border-emerald-500/30 shadow-lg shadow-emerald-500/5" : "bg-white/[0.01] border-border-dim hover:bg-white/[0.03] hover:border-emerald-500/10"
                              )}
                            >
                               <div className="flex justify-between items-start mb-1">
                                  <span className={cn("text-xs font-bold transition-colors truncate pr-2", activeChatPhone === chat.phone ? "text-emerald-500" : "text-white")}>
                                     {chat.phone === chat.name ? chat.phone : (chat.name || chat.phone)}
                                  </span>
                                  <span className="text-[9px] font-bold text-slate-600 shrink-0">{new Date(chat.lastTimestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                               </div>
                               <p className="text-[10px] text-slate-500 truncate pr-4">{chat.lastText}</p>
                            ...
                            <div className="py-20 text-center space-y-3 opacity-30">
                               <ChatCircleDots size={32} className="mx-auto" />
                      ...
                      {activeChatPhone ? (
                         <>
                            ...
                            <div className="p-4 lg:p-6 border-b border-border-dim flex items-center justify-between bg-white/[0.01]">
                               ...
                               <div className="flex items-center gap-2">
                                  <div className="px-3 py-1 bg-emerald-500/10 text-emerald-500 rounded-full text-[9px] font-bold uppercase tracking-widest border border-emerald-500/20">24h Window Active</div>
                               </div>
                            </div>
                            ...
                            <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar flex flex-col">
                               {activeChatHistory.map((msg, i) => (
                                  <div key={i} className={cn("max-w-[80%] flex flex-col", msg.from === 'me' ? "self-end items-end" : "self-start items-start")}>
                                     <div className={cn(
                                        "px-4 py-3 rounded-2xl text-xs font-medium leading-relaxed shadow-sm whitespace-pre-wrap break-words",
                                        msg.from === 'me' ? "bg-emerald-500 text-black rounded-tr-none" : "bg-white/5 text-slate-200 border border-border-dim rounded-tl-none"
                                     )}>
                                        {msg.text}
                                     </div>
                                     <span className="text-[8px] font-bold text-slate-600 mt-1.5 uppercase tracking-widest px-1">
                            ...
                            <form onSubmit={handleSendReply} className="p-4 lg:p-6 bg-white/[0.01] border-t border-border-dim">
                               <div className="relative flex items-end gap-3">
                                  <textarea 
                                    ...
                                    className="flex-1 bg-bg-base border border-border-dim rounded-2xl p-4 text-xs font-medium text-white outline-none focus:border-emerald-500/30 resize-none min-h-[56px] max-h-32 transition-all"
                                    onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSendReply(e); } }}
                                  />
                                  <button 
                                    type="submit" 
                                    disabled={!replyText.trim() || isSendingReply}
                                    className="h-14 px-6 bg-emerald-500 text-black rounded-2xl font-bold text-xs uppercase tracking-widest hover:scale-[1.02] active:scale-95 transition-all shadow-lg shadow-emerald-500/10 disabled:opacity-50 disabled:hover:scale-100"
                                  >
                   ...
                   <div className="simple-card space-y-8 lg:space-y-10 p-6 lg:p-8">
                      <div className="flex items-center justify-between border-b border-border-dim pb-4 lg:pb-6">
                          <div className="flex items-center gap-3 lg:gap-4">
                             ...
                             <div>
                                <h3 className="text-base lg:text-lg font-bold">API Settings</h3>
                                <p className="text-[10px] lg:text-xs text-slate-500">Connect your Meta account.</p>
                             </div>
                          </div>
                          <button 
                            type="button" 
                            onClick={() => setRevealCredentials(!revealCredentials)} 
                            className={cn("flex items-center gap-2 px-3 py-1.5 rounded-lg border transition-all text-[10px] font-bold uppercase tracking-widest", revealCredentials ? "bg-amber-500/10 text-amber-500 border-amber-500/20" : "bg-white/5 text-slate-400 border-border-dim hover:text-white")}
                          >
                      ...
                      <form className="space-y-6 lg:space-y-8" onSubmit={async e => {
                        ...
                        setStatus('Saving...');
                        try {
                           const res = await fetch(`${API_BASE}/api/config`, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(config) });
                           if (res.ok) {
                             setIsConnected(true);
                             setRevealCredentials(false);
                             setStatus('Settings saved! Refreshing templates...');
                             await handleRefreshTemplates();
                         ...
                         <div className="space-y-5 lg:space-y-6">
                            ...
                            <div className="space-y-2 relative group">
                               ...
                               <textarea 
                                 ...
                                 className={cn(
                                   "w-full h-28 lg:h-32 bg-bg-surface border border-border-dim rounded-xl p-4 text-xs font-mono outline-none focus:border-emerald-500/30 resize-none transition-all",
                                   !revealCredentials ? "text-transparent select-none filter blur-[2px]" : "text-white"
                                 )} 
                                 placeholder="Paste your Meta Graph API access token..." 
                               />
                               {!revealCredentials && config.ACCESS_TOKEN && (
                                 <div className="absolute inset-0 top-6 flex items-center justify-center pointer-events-none">
                                   <span className="text-[10px] font-bold text-slate-700 uppercase tracking-[0.5em] tracking-widest px-4 py-2 border border-dashed border-slate-800 rounded-lg">CREDENTIALS MASKED</span>
                                 </div>
                               )}
                            </div>
                         </div>
                       ...
                       <div className="pt-8 border-t border-border-dim space-y-6">
                          <div className="flex items-center gap-3">
                             ...
                             <div>
                                <h4 className="text-sm font-bold">Meta Phone Registration</h4>
                                <p className="text-[10px] text-slate-500">Only needed if your dashboard shows "Pending".</p>
                             </div>
                          </div>
                          <div className="space-y-4 bg-white/[0.01] p-5 rounded-2xl border border-border-dim">
                             <div className="space-y-2">
                                ...
                                <input 
                                  ...
                                  className="w-full bg-bg-surface border border-border-dim rounded-xl p-3 text-center text-xl font-mono tracking-[0.5em] text-white outline-none focus:border-amber-500/30 font-bold" 
                                  placeholder="000000" 
                                />
                             </div>
                             <button 
                               ...
                               className="simple-btn bg-amber-500 text-black w-full h-12 font-bold flex items-center justify-center gap-2 shadow-lg shadow-amber-500/10 disabled:opacity-50"
                             >
        ...
        <footer className="h-10 border-t border-border-dim px-6 lg:px-10 flex items-center justify-between text-[10px] font-bold text-slate-600 uppercase tracking-widest shrink-0">
           <div>© 2026 TIM WhatsApp API</div>
           <div className="flex items-center gap-4">
              <span className="flex items-center gap-1.5"><div className="w-1.5 h-1.5 rounded-full bg-emerald-500" /> PRO MODE</span>
              <span>v1.2.0</span>
           </div>
        </footer>
      </main>
                 ...
                 <div className="p-6 lg:p-8 space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                       ...
                       <div className="p-4 rounded-2xl bg-white/[0.02] border border-border-dim">
                          ...
                          <p className={cn("text-sm font-bold", selectedResult.status?.includes('Sent') ? "text-emerald-500" : "text-red-500")}>{selectedResult.status}</p>
                       </div>
                    </div>
  ...
  return (
    <div className="space-y-2 pointer-events-auto">
       ...
       <input 
         ...
         className={cn(
           "w-full bg-bg-surface border border-border-dim rounded-xl p-3 lg:p-4 text-sm font-bold text-white outline-none focus:border-emerald-500/30 transition-all placeholder:text-slate-800",
           isSensitive && "tracking-[0.5em] font-mono"
         )} 
       />
    </div>
  );
